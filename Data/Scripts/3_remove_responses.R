...
flags$EXCLUDE3 <- (
  (!is.na(Data$ACCEPT) & Data$ACCEPT == "Decline") | 
...
flags$EXCLUDE4 <- (
  !is.na(Data$term) & 
  Data$term %in% c("Q1.2_Consent", "Q1.4_Age", "Q1.5_NotUS",
                   "Q1.6_NotCommuting", "Speeder"))
...
table(temp$ExpTime / 60 < 5 | 
      temp$ExpTime / 60 > 60) # TRUE = 11, FALSE = 722
...
table(rowSums(d, na.rm=T) < 45 | 
      rowSums(d, na.rm=T) / 60 > 20) # TRUE = 9, FALSE = 724
...
flags$FLAG3E <- (!is.na(Data$TCPD) & !is.na(Data$INPD) & 
                (Data$TCPD > Data$INPD))
...
table((Data$INPD / (Data$WTPD / 60) < 4) | 
...
# combined flag
flags$FLAG3 <- (flags$FLAG3A | flags$FLAG3B | flags$FLAG3C |
                flags$FLAG3D | flags$FLAG3E | flags$FLAG3F)
...
flags$FLAG4A <- (!is.na(Data$CDAYS) & !is.na(Data$WDAYS) &
...
table(Data$AGECAT != Data$AGE2)
flags$FLAG4B <- (!is.na(Data$AGECAT) & !is.na(Data$AGE2) &
...
Data$PERINCCAT <- cut(Data$PERINC,
                      breaks=c(0, 15000, 50000, 100000, 150000, 1000000000), 
                      right=F)
