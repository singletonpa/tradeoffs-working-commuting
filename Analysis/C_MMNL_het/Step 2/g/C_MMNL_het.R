...
# Construct AT
database$MODE_Active <- ifelse(database$MODE_Walk==1 | database$MODE_Bike==1, 1, 0)
...
apollo_randCoeff <- function(apollo_beta, apollo_inputs) {
  ...
  randcoeff[["g_TT"]] =  exp(m_g_TT + s_g_TT * draws_g_TT + 
                               bt_age_60up*AGE2_60_UP + bt_race_oth*RACE_Other + 
                               bt_hh_child*HHKIDS + 
                               bt_inc_0050*HHINC_000_050 + bt_inc_100p*HHINC_100__UP + 
                               bt_pt_subsu*PLACETYPE_Suburban + bt_pt_rural*PLACETYPE_Rural_all + 
  ...
  randcoeff[["g_WT"]] =  exp(m_g_WT + s_g_WT * draws_g_WT + s_TTWT * draws_g_TT + 
                               bw_age_60up*AGE2_60_UP + bw_race_oth*RACE_Other + 
...
apollo_deltaMethod(model, deltaMethod_settings=list(
  operation="lognormal", parName1="m_b_TC", parName2="s_b_TC"
))
apollo_deltaMethod(model, deltaMethod_settings=list(
  operation="lognormal", parName1="m_g_TT", parName2="s_g_TT"
))
apollo_deltaMethod(model, deltaMethod_settings=list(
  operation="lognormal", parName1="m_g_WT", parName2="s_g_WT"
))
apollo_deltaMethod(model, deltaMethod_settings=list(
  operation="lognormal", parName1="m_g_IN", parName2="s_g_IN"
))
...
save(unconditionals, file = "C_unconditionals.rds")

# Calculate mean, standard deviation, and median
# of willingness-to-pay coefficients
# g_TT
mean(as.vector(unconditionals$g_TT)) * 60
sd(as.vector(unconditionals$g_TT)) * 60
median(as.vector(unconditionals$g_TT)) * 60
# g_WT
mean(as.vector(unconditionals$g_WT)) * 60
sd(as.vector(unconditionals$g_WT)) * 60
median(as.vector(unconditionals$g_WT)) * 60
# correlation (g_TT, g_WT)
cor(rowMeans(unconditionals$b_TC), rowMeans(unconditionals$g_TT))
cor(rowMeans(unconditionals$b_TC), rowMeans(unconditionals$g_WT))
cor(rowMeans(unconditionals$b_TC), rowMeans(unconditionals$g_IN))
cor(rowMeans(unconditionals$g_TT), rowMeans(unconditionals$g_WT))
cor(rowMeans(unconditionals$g_TT), rowMeans(unconditionals$g_IN))
cor(rowMeans(unconditionals$g_WT), rowMeans(unconditionals$g_IN))
...
rm(conditionals, unconditionals)
gc()
